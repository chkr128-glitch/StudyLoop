...
次に、この新機能を使うための **`index.html`** （ストア画面の追加）と **`main.js`** （初期化のつなぎこみ）、そして単語帳画面からストアを開くための **`flashcard.js`** の微修正を行います。

準備ができましたら「UIのコードを生成して」とお伝え下さい！
（ちなみに、運営用の公式パックは、まずはブラウザのFirebaseコンソールから直接手動で追加してテストできるように、あとでサンプルのデータ形式をお渡ししますね）