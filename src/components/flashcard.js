...
// HTML側のonclick属性から直接呼び出せるように、グローバルに公開する（段階的移行のため）
export function initFlashcard() {
    window.showFcView = showFcView;
    window.deleteFcSet = deleteFcSet;
    window.openFcSet = openFcSet;
    window.deleteFcWord = deleteFcWord;
    window.startFcLearning = startFcLearning;
    window.flipFcCard = flipFcCard;
    window.evalFcWord = evalFcWord;
...
// データベースの監視(onSnapshot)から呼ばれ、内部データを更新する
